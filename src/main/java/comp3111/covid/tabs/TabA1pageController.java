...
public class TabA1pageController extends TableTabsController{
	...
	protected ArrayList<TableData> generateDataList(ArrayList<String> isoCodes, LocalDate targetDate) throws Exception{
		...
		for(String isoCode : isoCodes) {
			long totalData = db.searchTotalData(isoCode, targetDate, DataTitle.CASE); // search all case data
			double rateData = db.searchRateData(isoCode, targetDate, DataTitle.CASE); // search all case rate data
	...
	protected void generateTable(ArrayList<TableData> data, LocalDate date) {
		...
		this.tableTitlelbl.setText("Number of Confirmed COVID-19 Cases as of " + date);
